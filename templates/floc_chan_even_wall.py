...
class EvenWall:
    """This is a class for every other wall starting with the second wall ("even" wall).
    It's called by the flocculator class so that the hierarchy of objects in
    Python is the same as in Fusion.

    Attributes
    ----------
    These are the default values for an "even" wall assembly. To overwrite, pass
    these into the bod (Basis Of Design) variable into the constructor.
